...
#Bottle Model
model_bottle_size = tf.keras.models.load_model('model/bottle_size_model.h5') 
model_bottle_brand = tf.keras.models.load_model('model/bottle_brand_model.h5')  
...
#Can Model
model_can_size = tf.keras.models.load_model('model/can_size_model.h5') 
model_can_brand = tf.keras.models.load_model('model/can_brand_model.h5')  
...
# If the confidence score is below this value, the result will be classified as "unknown"
# Reduced from 0.5 to 0.3 to allow more predictions
...
def predict_category(image_bytes, model_size, model_brand, size_classes, brand_classes):
    ...
    #----------------------------------Debug-----------------------------------
    print(f"Size Prediction: {size_name} (Confidence: {size_confidence})")
    print(f"Brand Prediction: {brand_name} (Confidence: {brand_confidence})")
    #--------------------------------------------------------------------------
